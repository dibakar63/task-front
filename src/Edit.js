...
const Edit = () => {
    ...
    const handleSubmit=async(e)=>{
        e.preventDefault();
        try {
          const res=await  axios.put(`https://task-back-3v66.onrender.com/api/v1/update/${id}`,{name,contact,address})
  ...
  return (
    <div className='main'>
      <div className='container'>
        <div className='login'>
        <div className='card'>
        <h1 style={{marginTop:"40px",fontSize:"60px"}}>Edit</h1>
        ...
        <input type='text' placeholder='Enter your name' style={{width:"300px",height:"35px",borderRadius:"15px",textAlign:"center"}} value={name} onChange={(e)=>setName(e.target.value)} required/>
           ...
           <textarea type='text' placeholder='Enter your address' rows="4" cols="50" style={{width:"300px",height:"75px",borderRadius:"15px",border:"none",textAlign:"center",marginTop:"20px"}} value={address} onChange={(e)=>setAddress(e.target.value)} required/>
            ...
            <button type='submit' style={{marginTop:"20px",width:"300px",height:"35px",borderRadius:"15px",backgroundColor:"green",font:"white",color:"white",fontSize:"20px"}} onClick={handleSubmit}>Update</button>
